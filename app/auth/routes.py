...
from app.auth.email import send_pass_reset_email
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(_('Congratulation, you are register in system'))
        return redirect(url_for('auth.login'))
...
@bp.route('/reset_pass/<token>', methods=['GET', 'SET'])
def reset_pass(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash(_('Your pass has been reset.'))
        return redirect(url_for('auth.login'))
